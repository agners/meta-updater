...
IMAGE_CMD_ota-ext4 () {
	...
	rm -rf ${DEPLOY_DIR_IMAGE}/${IMAGE_NAME}.otaimg.ext4
	sync
	dd if=/dev/zero of=${DEPLOY_DIR_IMAGE}/${IMAGE_NAME}.otaimg.ext4 seek=${OTA_ROOTFS_SIZE} count=$COUNT bs=1024
	mkfs.ext4 -O ^64bit ${DEPLOY_DIR_IMAGE}/${IMAGE_NAME}.otaimg.ext4 -L otaroot -d ${OTA_SYSROOT}
	rm -f ${DEPLOY_DIR_IMAGE}/${IMAGE_LINK_NAME}.otaimg.ext4
	ln -s ${IMAGE_NAME}.otaimg.ext4 ${DEPLOY_DIR_IMAGE}/${IMAGE_LINK_NAME}.otaimg.ext4
}
